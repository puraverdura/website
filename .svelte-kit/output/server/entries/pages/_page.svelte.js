import { f as attr, l as escape_html, n as head, k as ensure_array_like, c as pop, p as push } from "../../chunks/index.js";
...
function WTeaser($$payload, $$props) {
  ...
  $$payload.out += `<div><a class="!m-0 !p-0"${attr("href", link)}><img${attr("src", imageSrc)}${attr("alt", imageAlt)}> <h2 class="mt-[20px] mb-[3px] leading-[30px]">${html(title)}</h2></a> <p class="pt-[5px]">${escape_html(text)}</p> <a class="mt-[6px] md:mt-[12px]"${attr("href", link)}>mehr erfahren »</a></div>`;
}
function _page($$payload, $$props) {
  ...
  if (data.eventsForHomepage.length) {
    ...
    $$payload.out += `<!--]--> <div class="flex flex-col items-end mt-[14px] sm:mt-[17px]"><a class="rounded-[4px] bg-primary-15 !px-[5px] !font-heading" href="/agenda/">alle Termine »</a></div></div>`;
  } else {
    $$payload.out += "<!--[!-->";
  }
  $$payload.out += `<!--]--></div></div>`;
