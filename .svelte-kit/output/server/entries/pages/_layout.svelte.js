...
function NavigationMenu($$payload, $$props) {
  ...
  $$payload.out += `<div${attr("class", `${stringify(className)} top-0 left-0 bottom-0 right-0 z-99 bg-background-100 overflow-y-auto overscroll-y-contain`)} style="scrollbar-width: none;">`;
  Header($$payload, {
  ...
  for (let index = 0, $$length = each_array.length; index < $$length; index++) {
    ...
    $$payload.out += `<li class="h1 flex justify-center text-center border-b-[1px] border-b-primary-100 font-bold text-[32px] text-primary-100 !py-[6px]"><button class="cursor-pointer hover:text-primary-120"${attr("tabindex", index + 1)}>${escape_html(item.name)} »</button></li>`;
  }
